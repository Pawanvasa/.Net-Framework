...
namespace APIApps.Services
{
    public class ProductDataAccessService : IDbAccessService<Product, int>
    {
        ...
        public ProductDataAccessService(eShoppingContext Context)
        {
        ...
        public async Task<IEnumerable<Product>> getProdByCatID(int id)
        {
            var product = (await context.Products.ToListAsync()).Where(p => p.CategoryId == id);
            return product;
        }
    }
}
